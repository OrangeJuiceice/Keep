...
export default {
  ...
  onLoad() {
    ...
    if (uni.getStorageSync("timer1")) {
      this.groups = uni.getStorageSync("timer1")[0];
      this.times = uni.getStorageSync("timer1")[1];
      this.rest = uni.getStorageSync("timer1")[2];
    }
  ...
  methods: {
    bindTimeChange(e) {
      ...
      uni.setStorageSync("timer1", [this.groups, this.times, this.rest]);
    },
    countDown() {
      this.times--;
      ...
      if (this.times <= 0 && this.groups == 1) {
        ...
        // 单次运动时间
        var seconds =
          uni.getStorageSync("timer1")[0] * uni.getStorageSync("timer1")[1] * 2;
        ...
        uni.showModal({
          ...
          success: function (res) {
            if (res.confirm) {
              console.log("用户点击确定");
              uni.navigateTo({
                url: "/pages/edit/index?move=肱二头肌弯举&time=" + time,
              });
            } else if (res.cancel) {
              console.log("用户点击取消");
            }
            ...
            var total =
              ...
              parseInt(uni.getStorageSync("timer1")[0]) *
                parseInt(uni.getStorageSync("timer1")[1]) *
                2;
